...
class HostsConfigParser:
    ...
    def __init__(self, config_file):
        ...
        self.groups = {}
        self.port_ranges = {}
        self.guid_port_ranges = {}  # 新增：存储GUID到端口范围的映射
        self.parse_config()
    ...
    def parse_config(self):
        ...
        for line in lines:
            ...
            if line.startswith('0x') and current_group:
                guid = line.lower()
                self.groups[current_group].append(guid)
                # 为每个GUID记录其端口范围
                if current_port_range:
                    self.guid_port_ranges[guid] = current_port_range
    ...
    def get_port_range_for_guid(self, guid):
        """获取指定GUID的端口范围"""
        return self.guid_port_ranges.get(guid.lower(), None)
    ...
    for item in data:
        ...
        for group_name in groups_to_process:
            ...
            if item_guid in group_guids:
                ...
                guid_port_range = config_parser.get_port_range_for_guid(item_guid)
                group_port_range = config_parser.get_port_range(group_name)

                # 优先使用GUID特定的端口范围
                port_range = guid_port_range if guid_port_range else group_port_range
    ...
    if config_parser:
        ...
        for group_name in target_groups:
            ...
            for item in group_data:
                guid_range = config_parser.get_port_range_for_guid(item['source_guid'])
                if guid_range:
                    guid_ranges.add(guid_range)

            if len(guid_ranges) > 1:
                # 多个端口范围，显示为混合范围
                range_list = sorted(list(guid_ranges))
                range_str = f" (混合端口范围: {', '.join([f'{r[0]}-{r[1]}' for r in range_list])})"
            elif group_port_range:
                range_str = f" (端口范围: {group_port_range[0]}-{group_port_range[1]})"
            elif len(guid_ranges) == 1:
                r = list(guid_ranges)[0]
                range_str = f" (端口范围: {r[0]}-{r[1]})"
            else:
                range_str = ""
            ...
            if group_down:
                # 按设备统计Down端口，格式: GUID 设备名: 端口 X, Y, Z
                down_by_device = defaultdict(list)
                for item in group_down:
                    device_key = (item['source_guid'], item['source_name'])
                    down_by_device[device_key].append(item['source_port'])

                for (guid, device_name), ports in down_by_device.items():
                    ports_str = ', '.join(sorted(ports, key=int))
                    print(f"      {guid}  {device_name}: 端口 {ports_str}")

def main():
    # 命令行参数解析
    parser = argparse.ArgumentParser(
        description='Author: VincentWu@zhengytech.com\nIB LinkInfo Parser - 解析InfiniBand链路信息',
        formatter_class=argparse.RawDescriptionHelpFormatter
    )
    #parser = argparse.ArgumentParser(description='Author: VincentWu@zhengytech.com\\nIB LinkInfo Parser - 解析InfiniBand链路信息')
    ...
    parser.add_argument('-f', '--file', help='从指定文件读取iblinkinfo输出结果')
    ...
    # 检查参数冲突
    if args.file and args.ca_name:
        print("错误：-f/--file 和 ca_name 参数不能同时使用")
        sys.exit(1)
    ...
    if args.file:
        # 从文件读取数据
        output = read_from_file(args.file)
    else:
        # 从命令行执行获取数据
        if args.ca_name:
    ...
    if args.file:
        ...
        base_name = f"{prefix}_{file_basename}_{timestamp}"
    elif args.ca_name:
