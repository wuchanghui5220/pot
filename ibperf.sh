...
if [ "$ENABLE_NUMA_BINDING" = true ]; then
    ...
    # 对每个主机检测 MST 并获取 NUMA 映射
    for host in "${unique_hosts[@]}"; do
        # 检查并启动 MST
        mst_status=$(ssh_cmd "$host" "mst status 2>/dev/null | grep -c 'MST PCI'")
        if [ "$mst_status" = "0" ] || [ -z "$mst_status" ]; then
            ssh_cmd "$host" "mst start &>/dev/null"
            sleep 1
        fi
        ...
        # 解析 mst status -v 输出，提取 RDMA 设备名和 NUMA 节点
        while IFS= read -r line; do
...
for (( pair_idx=0; pair_idx<PAIR_COUNT; pair_idx++ )); do
    ...
    for hca_idx in "${!SERVER_HCAS[@]}"; do
        ...
        if [ $((server_count % MAX_CONCURRENT_BATCH)) -eq 0 ]; then
            wait
            echo -e "  ${CYAN}已启动 ${server_count}/${PAIR_COUNT} 对主机的服务端...${NC}"
            sleep 1
        fi
    done
done
...
for (( pair_idx=0; pair_idx<PAIR_COUNT; pair_idx++ )); do
    ...
    for hca_idx in "${!CLIENT_HCAS[@]}"; do
        ...
        if [ $((client_count % MAX_CONCURRENT_BATCH)) -eq 0 ]; then
            wait
            echo -e "  ${CYAN}已启动 ${client_count}/${PAIR_COUNT} 对主机的客户端...${NC}"
            sleep 1
        fi
    done
done
...
for (( pair_idx=0; pair_idx<PAIR_COUNT; pair_idx++ )); do
    ...
    for hca_idx in "${!SERVER_HCAS[@]}"; do
        ...
        scp -o StrictHostKeyChecking=no -o LogLevel=ERROR \
            ...
            "$LOG_DIR/${server}_${server_hca}_server.log" 2>/dev/null &

        # 收集客户端日志
        scp -o StrictHostKeyChecking=no -o LogLevel=ERROR \
            "$USER@$client:${REMOTE_LOG_DIR}/client_pair${pair_idx}_${client_hca}.log" \
            "$LOG_DIR/${client}_${client_hca}_client.log" 2>/dev/null &
