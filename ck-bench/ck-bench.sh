...
done < <(lldpcli show neighbors 2>/dev/null)

# Collect topology for each HCA (only active ones)
...
TOPO_SCRIPT_ROCE
    ...
    for host in "${hosts[@]}"; do
        (
            ...
            case ${RUN_ENV} in
                mac)
                    scp -q "${topo_script_file}" ${CPU_SERVER_USER}@${CPU_SERVER}:/tmp/ 2>/dev/null || true
                    result=$(ssh -n ${CPU_SERVER_USER}@${CPU_SERVER} "scp -q /tmp/$(basename ${topo_script_file}) ${host}:/tmp/ && ssh -n ${host} 'bash /tmp/$(basename ${topo_script_file}) \"${hca_filter}\"'" 2>/dev/null || true)
                    ;;
                cpu_server)
                    scp -q "${topo_script_file}" ${host}:/tmp/ 2>/dev/null || true
                    result=$(ssh -n ${host} "bash /tmp/$(basename ${topo_script_file}) '${hca_filter}'" 2>/dev/null || true)
                    ;;
                gpu_server)
                    scp -q "${topo_script_file}" ${host}:/tmp/ 2>/dev/null || true
                    result=$(ssh -n ${host} "bash /tmp/$(basename ${topo_script_file}) '${hca_filter}'" 2>/dev/null || true)
